...
Calls the Ollama server's `/api/show` endpoint for the specified model and returns a `ModelDefinition` with mapped SDK capabilities and full metadata. Capability mapping translates Ollama's native capability strings (`completion`, `vision`, `tools`, `thinking`, `embedding`, `image`) into typed `AiSdk\Capability` enum values. See [Model Discovery](/guides/model-discovery) for the full mapping table.
...
Clears the singleton provider by setting the internal `$default` property to `null`. The next call to any static method that requires a provider will lazily re-initialize it. This method is essential in test suites to prevent state leaking between test cases.
