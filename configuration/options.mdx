...
description: "Configure the Ollama provider with custom base URLs, API selection, HTTP headers, and SDK instance binding using Ollama::create() or OllamaOptions::fromArray()."
---
...
<ParamField body="headers" type="array" optional>
  Additional HTTP headers merged into every outbound request. When `apiKey` is also set, the `Authorization` header from `apiKey` is prepended first, so a conflicting `Authorization` entry in `headers` is overridden by `apiKey`.
</ParamField>
